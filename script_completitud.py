...
class DataCompletenessAuditor:
    ...
    def _refill_asset(self, cfg: TimeseriesTableConfig, asset_id: Any, asset_report: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for group in groups:
            ...
            fetched = self._fetch_range(cfg, symbol, epic, eligible_ts, start, end)
    ...
    def _fetch_range(
        ...
        if source == "capital":
            ...
            return self.capital.fetch_prices(epic, start.to_pydatetime(), end.to_pydatetime(), resolution)
        if source == "fmp":
            if not symbol:
            ...
            if not records and cfg.frequency == "daily" and HAS_YFINANCE:
                yf_records = self.yf_client.fetch(symbol, start.to_pydatetime(), end.to_pydatetime(), interval="1d")
                records.extend(yf_records)
            elif not records and cfg.frequency == "intraday" and HAS_YFINANCE:
                interval = "1m" if (cfg.interval_minutes or 1) <= 1 else f"{cfg.interval_minutes}m"
                records.extend(self.yf_client.fetch(symbol, start.to_pydatetime(), end.to_pydatetime(), interval=interval))
            return records
        return []
